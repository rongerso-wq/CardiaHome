...
- **Clinical zone engine** (`computeZone()` + `updateZone()`) — the core clinical logic, now **multi-vital**. Models: `weight { dry, today, min, max, hist[7] }`, `bp { sys, dia }`, `spo2 { val }` (BP + SpO₂ are the "auto-synced from the watch" vitals). Rule set: **weight** — ≥5 lb/day → Red; ≥3 lb/day, ≥3 lb over 2 days, ≥5 lb/week, or ≥5 lb above `dry` → Yellow; **BP** — ≥180/110 → Red, ≥160/100 → Yellow; **SpO₂** — <90 → Red, <94 → Yellow; **symptoms** — any `.choice.sel-bad` → ≥ Yellow. `computeZone()` returns `{ zone, reasons, sub:{weight,bp,spo2} }`; the overall zone is `worstOf(...)` all vitals + symptoms (thresholds follow ACC/AHA HF guidance — cited in-app). `updateZone()` recolors the hero, sets the `#alert` banner (Red/Yellow tails now say the **care team was auto-alerted** — the "send somewhere" escalation), and **badges each card by its OWN sub-zone** via `setBadge()`/`setStat()` (Scale, BP `#bp-badge`, Oxygen `#spo2-badge`, + the three panels). Each vital has a stepper (`setWeight`/`setBP`/`setSpo2`) to drive the zone live. Called from `refreshProgress()`.
